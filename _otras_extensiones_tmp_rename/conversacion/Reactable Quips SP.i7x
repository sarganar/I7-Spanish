Version 10.1 of Reactable Quips SP by Michael Martin begins here.

"A table-based approach to NPC conversation chains, as well as allowing rules to fire on lines of conversation."

"SPANISH Work by Sarganar - STAND ALONE VERSION."

[SPANISH port - 2026-09-11 - Re-based on Reactable Quips v10.1 (the Inform 10.1 compatibility fixes below are Nathanael Nerode's, not part of the Spanish port) instead of the older pre-10.1 baseline this translation used to carry. The old baseline had the "say [number]"/"[number]" Understand lines merged into a single "or" line and an untyped/old-style blank-row table syntax; both are exactly what 220522/220523 fixed for Inform 10.1, so this port now follows the current upstream body instead.]
...
Chapter SP - SPANISH STUFFS

Section 1 - Quips

Understand "mostrar" or "recapitular" as requesting a recap when RQ is active.
Understand "di [number]" or "elige [number]" or "elegir [number]" as responding with when RQ is active.

Section 5 - Text overrides

The generic ask quip is "No se me ocurre nada que decir sobre ese tema.".
The generic tell quip is "No se me ocurre nada que decir sobre ese tema.".
The RQ options prologue is "Las opciones disponibles son:".
The RQ reaction demand is "[bracket]Necesito que sigas el hilo de la conversación para avanzar. Ingresa un número o el comando MOSTRAR para volver a ver tus opciones.[close bracket]".
The RQ no options is "[bracket]No hay respuestas disponibles.[close bracket]".

The spanish basic RQ out of range rule is listed instead of the basic RQ out of range rule in the RQ out of range rules.
An RQ out of range rule for a number (called max) (this is the spanish basic RQ out of range rule): say "[bracket]Las opciones válidas van del 1 al [max]. Escribe MOSTRAR para volver a ver tus opciones.[close bracket][paragraph break]".


Reactable Quips SP ends here.
...
Example: * Security Consultant - Extracting useful information from an NPC.
	...
	*: "Security Consultant"
	...
	Include Reactable Quips SP by Michael Martin.
