Version 5 of Quip-Based Conversation SP by Michael Martin begins here.
...
Include Version 10 of Reactable Quips SP by Michael Martin.
...
The QBC talking cue is "[bracket]Utiliza HABLA CON para interactuar con los personajes.[close bracket]".

Section 3 - Initiating conversations

Understand "habla con/-- [something]" as talking to.
...
Check talking to (this is the spanish can only talk to talkables rule): 
	if the noun provides the property litany and the noun provides the property greeting, do nothing; 
	otherwise say "Por lo general, es mejor hablar con cosas vivas." instead.

Section 4 - Delivering lines

Understand "elige [number]" or "di [number]" as QBC responding with when QBC is active.

Section 5 - Recaps

Understand "mostrar" or "recapitular" as QBC recap when QBC is active.

QBC no conversation error is "[bracket]Ahora mismo no estás conversando con nadie.[close bracket]".


Quip-Based Conversation SP ends here.
