...
if (glk_gestalt(gestalt_Sound,0) && glk_gestalt(gestalt_SoundMusic,0))  { 
	...
	"Tu int@'erprete no soporta reproducci@'on de sonidos.";
...
if (glk_gestalt(gestalt_SoundVolume,0)) {
	if ((val <= 5) && (val >=0)) {
		...
		"Nivel de volumen inv@'alido: por favor el nivel est@'a entre 0 y 5.";
		!"Invalid Volume Level: please insert a value between 0 and 5.";
		}
	}
else {
	"Tu int@'erprete no soporta control de volumen.";
...
if (glk_gestalt(gestalt_Sound,0) && glk_gestalt(gestalt_SoundMusic,0))  { 
	...
	"Tu int@'erprete no soporta detener sonido.";
