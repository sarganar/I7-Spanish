...
[Inform 10.1.2 note: kits are now precompiled binary modules, so the old
"Replace Box__Routine; ... before/after \"Glulx.i6t\"." pair no longer works
("this Inform 6 directive is not supported in kits or '(-' inclusions").
The modern equivalent is "replacing" by symbol name on the Include itself,
no "before/after" needed. Likewise ".print_to_array()" was removed from the
unified text pipeline; measuring a printed line's length now goes through a
Glk memory stream, the same technique "Text Capture" by Eric Eve uses (and
that Spanish Language's own [Tu]/[plm] capture reuses internally).]
...
Array _GlulxBoxQuoteMeasureBuf_ --> 256;
...
[ Box__Routine maxlen arr
    ...
    for (i=0 : i<lines : i=i+1) {
      ! Measure the printed length of this line via a Glk memory stream:
      ! .print_to_array no longer exists in Inform 10's unified text pipeline.
      old_stream = glk_stream_get_current();
      new_stream = glk_stream_open_memory_uni(_GlulxBoxQuoteMeasureBuf_ + WORDSIZE, 255, 1, 0);
